...
export default function Navbar({ onMenuToggle }) {
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center px-5 gap-3 shrink-0 z-20">
      <button
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {user && (
          <div className="relative" ref={dropdownRef}>
            <button
              className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 hover:bg-slate-100 transition-colors border border-slate-200"
              onClick={() => setDropdownOpen((v) => !v)}
            >
              <div className="w-8 h-8 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-bold shrink-0 select-none">
                {initials}
              </div>
