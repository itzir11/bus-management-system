...
function RouteStopsPanel({ routeId, routeStops, stops }) {
  ...
  if (!ordered.length) {
    return <p className="text-sm text-slate-400 py-2 px-4">No stops configured.</p>;
  }
...
function AddRouteModal({ depots, stops, visibleDepotId, onClose, onSave }) {
  ...
  const [depotId, setDepotId] = useState(visibleDepotId || (depots[0]?.id ?? ""));
  ...
  function handleAddStop(stop) {
    ...
    setAddedStops((prev) => [
      ...prev,
      { stop_id: stop.id, stop_name: stop.name, planned_offset_min: Number(selectedOffset) },
    ]);
    setStopSearch("");
    setSelectedOffset(0);
  }
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!code.trim()) { setError("Route code is required."); return; }
    if (!name.trim()) { setError("Route name is required."); return; }
    if (!depotId) { setError("Please select a depot."); return; }
    if (addedStops.length < 2) { setError("Add at least 2 stops."); return; }
    setError("");
    onSave({ code: code.trim(), name: name.trim(), depotId, stops: addedStops });
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Add Stops</label>
                ...
                {stopSearch && (
                  <div className="absolute top-full mt-1 left-0 right-0 z-10 bg-white border border-slate-200 rounded-lg shadow-lg max-h-44 overflow-y-auto">
                    {filteredStops.length === 0 ? (
                      <div className="px-3 py-2 text-sm text-slate-400">No stops found</div>
                    ) : (
                      filteredStops.map((s) => {
                        const already = addedStops.some((x) => x.stop_id === s.id);
                        return (
                          <button
                            key={s.id}
                            type="button"
                          ...
                          >
                            {s.name}
                            {already && <span className="ml-1 text-xs text-slate-400">(added)</span>}
                          </button>
                        );
                      })
                    )}
            ...
            {addedStops.length > 0 && (
              <div className="border border-slate-200 rounded-lg divide-y divide-slate-100">
                {addedStops.map((s, idx) => (
                  <div key={s.stop_id} className="flex items-center gap-2 px-3 py-2">
                    ...
                    <span className="flex-1 text-sm text-slate-700 truncate">{s.stop_name}</span>
                    <span className="text-xs text-slate-400 tabular-nums">+{s.planned_offset_min} min</span>
                    <button
                      type="button"
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {visibleRoutes.length} route{visibleRoutes.length !== 1 ? "s" : ""} configured
            {isManager && ` for ${getDepotById(depots, user.depot_id)?.name ?? "your depot"}`}
          </p>
        </div>
        {canManage && (
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {visibleRoutes.length === 0 ? (
          <div className="text-center py-16 text-slate-400 text-sm">No routes found.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Stops</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Distance</th>
              ...
              <tbody>
                {visibleRoutes.map((route) => {
                  ...
                  return (
                    <tr key={route.id} className="border-b border-slate-50 last:border-0">
                      <td colSpan={6} className="p-0">
                        <div>
                          <div
                            className={`grid cursor-pointer hover:bg-slate-50 transition-colors ${isExpanded ? "bg-slate-50" : ""}`}
                            style={{ gridTemplateColumns: "96px 1fr 180px 80px 100px 120px" }}
                          ...
                          >
                            ...
                            <div className="px-4 py-3 text-slate-700 font-medium truncate">{route.name}</div>
                            <div className="px-4 py-3 text-slate-500 truncate">{depot?.name ?? route.depot_id}</div>
                            <div className="px-4 py-3 text-slate-600">{route.total_stops}</div>
                            <div className="px-4 py-3 text-slate-600">{route.distance_km > 0 ? `${route.distance_km} km` : "—"}</div>
      ...
      {showAddModal && (
        <AddRouteModal
          depots={isManager ? depots.filter((d) => d.id === user.depot_id) : depots}
          stops={stops}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-base font-semibold text-slate-800 mb-2">Delete Route</h3>
